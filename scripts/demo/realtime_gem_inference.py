...
class RealtimeGEMInference:
    ...
    def load_model(self):
        ...
        self._model = load_model(ckpt_path, exp_name="gem_smpl_regression")
    ...
    def run(self, data_dict: dict) -> InferenceResult:
        ...
        # 수동으로 병목(IK) 없는 Translation 후처리만 수행 (발이 땅에서 붕 뜨거나 틀어지는 문제 방지)
        if self._config.postprocess:
            from gem.utils.postprocess import pp_static_joint
            outputs = pred["net_outputs"]
            endecoder = self._model.pipeline.endecoder
            
            # 1~2ms 소요되는 고속 연산 (접촉 보정 및 y축 지면 정렬)
            refined_transl = pp_static_joint(outputs, endecoder)
            
            outputs["pred_body_params_global"]["transl"] = refined_transl
            pred["body_params_global"]["transl"] = refined_transl[0]
