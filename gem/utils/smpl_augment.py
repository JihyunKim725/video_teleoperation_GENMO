...
_coco_aug_path = _BODY_MODEL_DIR / "coco_aug_dict.pth"
if _coco_aug_path.exists():
    COCO17_AUG = {
        k: v.flatten()
        for k, v in torch.load(_coco_aug_path, weights_only=False).items()
    }
else:
    print(f"⚠️  {_coco_aug_path} 없음 — 빈 dict 사용 (추론에는 영향 없음)")
    COCO17_AUG = {}
COCO17_AUG_CUDA = {}
COCO17_TREE = [
    [5, 6],
